...
class TrajectoryController extends BaseAdminController
{
	...
	public function rent_trajectory_info($rent_id, $map_id = 0)
	{
		...
		if (empty($rent_info)) {
			echo '<h1>页面数据错误</h1><script>setTimeout(function(){location.href="' . route('admin.positions.rent_trajectory_list') . '"},2000);</script>';
			exit;
		}
		//获取有数据的map_id
	...
	public function backup_trajectory_info($rent_id, $map_id = 0)
	{
		...
		if (empty($rent_info)) {
			echo '<h1>页面数据错误</h1><script>setTimeout(function(){location.href="' . route('admin.positions.backup_trajectory_list') . '"},2000);</script>';
			exit;
		}
		//获取有数据的map_id
		$map_id_arr = Trajectory::where('rent_id', $rent_id)->groupBy('map_id')->pluck('map_id')->toArray();
